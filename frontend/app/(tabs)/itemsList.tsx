...
export default function ItemsListPage() {
    ...
    <ScrollView>
        ...
        {(items && items.length > 0) ? items.map((item : Item) => (
            <View key={item.item_id} style={{ marginBottom: 10 }}>
                {item.image_url && (
                    <Image 
                        source={{uri: item.image_url}} 
                        style={{ width: 200, height: 200, borderRadius: 10, marginBottom: 10, borderColor: "red", borderWidth: 10 }}
                        alt="TESTING"
                    />
                )}
                
                {
                    Object.keys(item).map((key : any) => {
                        ...
                        return (<View key={key}>
                            <Text style={{ fontWeight: 'bold', marginBottom: 5, color: '#fff' }}>{key.replace('_', ' ').toUpperCase() + ": " + item[key as keyof Item]}</Text>
                        </View>)
                    })
                }
            </View>
